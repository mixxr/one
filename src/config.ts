 ...
 */
let appName = process.env.SITECORE_JSS_APP_NAME || 'my-brand-one';

/**
 * The server.bundle.js file from your pre-built JSS app
 */

const bundlePath = process.env.SITECORE_JSS_SERVER_BUNDLE || `../dist/${appName}/server.bundle.js`;

const serverBundle = require(bundlePath) as ServerBundle;

httpAgentsConfig.setUpDefaultAgents(serverBundle);

const apiHost = process.env.SITECORE_API_HOST || 'https://xmc-sitecoresaa38a0-msfromxmcft3044-prodmike.sitecorecloud.io'; //|| 'https://edge.sitecorecloud.io/api/graphql/v1';

console.log('apiHost',apiHost);

appName = appName || serverBundle.appName;

const apiKey = process.env.SITECORE_API_KEY || serverBundle.apiKey; // || 'T0RxdHM1RDV2eFRMdDJRdGkvekgxMDBVZWxzTS82V1U1bWs4TVYxVWdTTT18c2l0ZWNvcmVzYWEzOGEwLW1zZnJvbXhtY2Z0MzA0NC1wcm9kbWlrZS0zZTk2';
...
export const config: ProxyConfig = {
  ...
  proxyOptions: {
    ...
    onProxyReq: (proxyReq, _req, _res) => {
      if (!proxyReq.path.includes('sc_site')) {
        proxyReq.path = proxyReq.path + '&sc_site=my-brand-one'
      }
    },
  },
  /**
